...
class WalletTradeModal extends StatefulWidget {
  final WalletAsset? marketAsset;
  final bool isBuy;
  final List<WalletAsset> additionalAssets;

  const WalletTradeModal({
    super.key,
    this.marketAsset,
    required this.isBuy,
  ...
  @override
  State<WalletTradeModal> createState() => _WalletTradeModalState();

}

class _WalletTradeModalState extends State<WalletTradeModal> {
  late bool _isBuy;
  ...
  void initState() {
    ...
    final stables = ['USDT', 'USDC'];
    final defaultAsset = _assets.where((a) => stables.contains(a.symbol.toUpperCase()) || a.isNative).firstOrNull ?? _assets.firstOrNull;
    
    final bus = UICommandBus.instance;
    WalletAsset? resolvedMarketAsset = widget.marketAsset;
    
    if (resolvedMarketAsset == null) {
      final payloadKey = _isBuy ? 'swap_to_token' : 'swap_from_token';
      final payload = bus.latestPayload(payloadKey);
      if (payload != null && payload['symbol'] != null) {
        final symbol = payload['symbol'].toString().toLowerCase();
        resolvedMarketAsset = _pickerAssets.where((a) => a.symbol.toLowerCase() == symbol).firstOrNull;
      }
    }
    
    if (_isBuy) {
      _toAsset = resolvedMarketAsset;
      _fromAsset = defaultAsset;
    } else {
      _fromAsset = resolvedMarketAsset;
      _toAsset = defaultAsset;
    }
    
    ScreenContextService.instance.setModal(_isBuy ? 'wallet_buy' : 'wallet_sell');
  ...
  void _toggleBuySell(bool isBuy) {
  ...
  Widget build(BuildContext context) {
    ...
    final assetPrice = _isBuy ? _toAsset?.priceUsd : _fromAsset?.priceUsd;
                ...
                children: [
                    ...
                    children: [
                      Expanded(
                        child: Column(
                          ...
                          children: [
                            Text(
                              '${PrivyChainRegistry.getChain(_swapChainKey).displayName ?? _swapChainKey.toUpperCase()} Wallet',
                            ...
                            Text(
                              _isBuy 
                                  ? (isRu ? 'Купить $toSymbol' : 'Buy $toSymbol')
                                  : (isRu ? 'Продать $fromSymbol' : 'Sell $fromSymbol'),
                  ...
                  Row(
                    ...
                    children: [
                      Text(
                        isRu
                            ? 'Цена: \$${assetPrice?.toStringAsFixed(4) ?? '0.00'}'
                            : 'Price: \$${assetPrice?.toStringAsFixed(4) ?? '0.00'}',
                        style: GuardianTextStyles.caption,
                      ),
                      Text(
                        isRu
                            ? 'Доступно: ${fromBalance?.toStringAsFixed(4) ?? '0.00'} $fromSymbol'
                            : 'Available: ${fromBalance?.toStringAsFixed(4) ?? '0.00'} $fromSymbol',
                  ...
                  Text(
                    _isBuy
                        ? (isRu ? 'Сумма покупки ($fromSymbol)' : 'Buy amount ($fromSymbol)')
                        : (isRu ? 'Количество $fromSymbol' : 'Amount ($fromSymbol)'),
                    style: GuardianTextStyles.caption.copyWith(
                      color: GuardianColors.textSecondary,
                      fontWeight: FontWeight.bold,
                    ),
                      ...
                      suffixIcon: fromBalance != null && fromBalance > 0
                          ? TextButton(
                              onPressed: () {
                                ...
                                setState(() {
                                  _amountController.text = fromBalance.toStringAsFixed(4);
                                });
                              },
                              child: const Text('MAX', style: TextStyle(color: GuardianColors.accent, fontWeight: FontWeight.bold)),
                            )
                          : null,
                    ),
                  ...
                  // ── Preview/Total Value ──────────────────────────────────────
                  if (_inputAmount > 0 && _estimatedOutput != null) ...[
                    Row(
                      mainAxisAlignment: MainAxisAlignment.spaceBetween,
                      children: [
                        Text(
                          isRu ? 'Получите (примерно):' : 'Receive (approximate):',
                          style: GuardianTextStyles.caption.copyWith(color: GuardianColors.textSecondary),
                        ),
                        Text(
                          '${_estimatedOutput!.toStringAsFixed(6)} $toSymbol',
                          style: GuardianTextStyles.caption.copyWith(
                  ...
                  Container(
                    ...
                    child: Column(
                      children: [
                        ...
                        const SizedBox(height: 6),
                        GestureDetector(
                          onTap: () => setState(() => _showSlippageOptions = !_showSlippageOptions),
                          child: Row(
                            mainAxisAlignment: MainAxisAlignment.spaceBetween,
                            children: [
                              Text(
                                isRu ? 'Допуск проскальзывания' : 'Slippage Tolerance',
                                style: GuardianTextStyles.caption.copyWith(color: GuardianColors.textSecondary),
                              ),
                              Row(
                                children: [
                                  Text('$_slippage%', style: const TextStyle(color: Colors.white, fontWeight: FontWeight.bold)),
                                  const SizedBox(width: 4),
                                  Icon(
                                    _showSlippageOptions ? Icons.keyboard_arrow_up : Icons.keyboard_arrow_down,
                                    size: 16,
                              ...
                              return Expanded(
                                child: GestureDetector(
                                  ...
                                  child: AnimatedContainer(
                                    ...
                                    decoration: BoxDecoration(
                                      color: isActive ? GuardianColors.accent : const Color(0xFF161B26),
                        ...
                        const SizedBox(height: 6),
                        _FeeRow(
                          label: isRu ? 'Сетевая комиссия' : 'Network Fee',
                          value: '\$${_estimatedNetworkFeeUsd.toStringAsFixed(2)}',
                        ),
                        const SizedBox(height: 6),
                        _FeeRow(
                          label: isRu ? 'Влияние на цену' : 'Price Impact',
                          value: '${_estimatedPriceImpactPct.toStringAsFixed(2)}%',
                        ),
                      ],
                    ),
                  ),
  ...
  Widget _buildSwapFlowSection(String fromSymbol, String toSymbol) {
    ...
    switch (_flowState) {
            ...
            SizedBox(
              ...
              child: ElevatedButton(
                ...
                style: ElevatedButton.styleFrom(
                  ...
                  backgroundColor: _isBuy ? GuardianColors.success : GuardianColors.danger,
                ...
                child: Text(
                  _isBuy 
                      ? (isRu ? 'Купить ${widget.marketAsset?.symbol ?? _toAsset?.symbol ?? ''}' : 'Buy ${widget.marketAsset?.symbol ?? _toAsset?.symbol ?? ''}')
                      : (isRu ? 'Продать ${widget.marketAsset?.symbol ?? _fromAsset?.symbol ?? ''}' : 'Sell ${widget.marketAsset?.symbol ?? _fromAsset?.symbol ?? ''}'),
          ...
          children: [
            ...
            SizedBox(
              ...
              child: ElevatedButton(
                ...
                child: Text(
                  _isBuy ? 'Продолжить покупку' : 'Продолжить продажу',
                  style: const TextStyle(
                      fontSize: 16, fontWeight: FontWeight.w700),
                ),
              ),
      ...
      case _SwapFlowState.swapping:
        ...
        label: _isBuy ? 'Выполняю покупку...' : 'Выполняю продажу...',
          showSpinner: true,
        );
      ...
      children: [
        ...
        SizedBox(
          ...
          child: ElevatedButton(
            ...
            child: Text(
              _isBuy ? 'Подтвердить покупку' : 'Подтвердить продажу',
        ...
        Container(
          ...
          child: Column(
            children: [
              ...
              const SizedBox(height: 8),
              Text(
                _isBuy ? 'Покупка выполнена' : 'Продажа выполнена',
